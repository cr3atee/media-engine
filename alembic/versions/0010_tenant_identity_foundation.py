...
Revision ID: 0010_tenant_identity_foundation
...
revision = "0010_tenant_identity_foundation"
...
TENANT_FOREIGN_KEYS = {
    "products": "fk_products_tenant_id_tenants",
    "prices": "fk_prices_tenant_id_tenants",
    "canonical_products": "fk_canonical_products_tenant_id_tenants",
    "offers": "fk_offers_tenant_id_tenants",
    "price_snapshots": "fk_price_snapshots_tenant_id_tenants",
    "market_events": "fk_market_events_tenant",
    "generated_contents": "fk_generated_contents_tenant",
    "publications": "fk_publications_tenant",
    "admin_actions": "fk_admin_actions_tenant",
}
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("length(btrim(email)) > 0", name="ck_users_email_nonempty"),
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("version", sa.Integer(), nullable=False),
        sa.CheckConstraint("length(btrim(name)) > 0", name="ck_tenants_name_nonempty"),
        sa.CheckConstraint("length(btrim(slug)) > 0", name="ck_tenants_slug_nonempty"),
    ...
    op.execute(
        sa.text(
            ...
            """,
        ).bindparams(legacy_tenant_id=LEGACY_TENANT_ID),
    )
    ...
    for table_name in TENANT_TABLES:
        ...
        op.execute(
            sa.text(
                f"UPDATE {table_name} SET tenant_id = :legacy_tenant_id "
                "WHERE tenant_id IS NULL",
            ).bindparams(legacy_tenant_id=LEGACY_TENANT_ID),
        )
        op.alter_column(table_name, "tenant_id", nullable=False)
        op.create_foreign_key(
            TENANT_FOREIGN_KEYS[table_name],
            table_name,
            "tenants",
            ["tenant_id"],
            ["id"],
    ...
    op.add_column("admin_actions", sa.Column("actor_type", sa.String(length=32)))
    op.add_column("admin_actions", sa.Column("elevated", sa.Boolean()))
    op.execute(
        "UPDATE admin_actions SET actor_type = 'api_key' WHERE actor_type IS NULL"
    )
    op.execute("UPDATE admin_actions SET elevated = false WHERE elevated IS NULL")
    op.alter_column("admin_actions", "actor_type", nullable=False)
    op.alter_column("admin_actions", "elevated", nullable=False)
    ...
    _rebuild_tenant_scoped_deterministic_keys()
    _replace_global_constraints_with_tenant_scoped()


def downgrade() -> None:
    _restore_global_constraints()
...
def _replace_global_constraints_with_tenant_scoped() -> None:
    op.drop_index(
    ...
    op.drop_index("ix_price_snapshots_history_order", table_name="price_snapshots")
    op.drop_constraint(
    ...
    op.create_index(
        "uq_market_events_snapshot_transition",
        "market_events",
        ["tenant_id", "event_type", "previous_snapshot_id", "current_snapshot_id"],
        unique=True,
        postgresql_where=sa.text(
            "previous_snapshot_id IS NOT NULL AND current_snapshot_id IS NOT NULL"
        ),
    ...
    op.create_index(
        ...
        "market_events",
        ["tenant_id", "marketplace", "external_id", sa.text("occurred_at DESC")],
        unique=False,
    )
    op.create_index(
    ...
    op.drop_index(
        "ix_generated_contents_event_created", table_name="generated_contents"
    )
    op.drop_constraint(
    ...
    op.create_index(
        ...
        "admin_actions",
        ["tenant_id", "actor_id", sa.text("created_at DESC"), sa.text("id DESC")],
        unique=False,
    )


def _rebuild_tenant_scoped_deterministic_keys() -> None:
    op.execute(
        sa.text(
            r"""
            ...
            IMMUTABLE
            AS $$
                SELECT replace(replace(value, E'\\', E'\\\\'), '|', E'\\|')
            $$;
            """,
        ),
    )
...
def _restore_global_constraints() -> None:
    ...
    op.drop_index("ix_canonical_products_tenant_name", table_name="canonical_products")
